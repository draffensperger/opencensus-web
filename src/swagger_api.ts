...
const BASE_PATH = "http://opencensusd.endpoints.opencensus-web-test.cloud.goog".replace(/\/+$/, "");
...
export interface MetricsDistributionValue {
    ...
    mean?: number;
    /**
     * Sum[i=1..n]((x_i - mean)^2)  Knuth, \"The Art of Computer Programming\", Vol. 2, page 323, 3rd edition describes Welford's method for accumulating this sum in one pass.  If count is zero then this field must be zero.
     * @type {number}
     * @memberof MetricsDistributionValue
     */
    sumOfSquaredDeviation?: number;
    /**
     * (-infinity, bucket_bounds[i]) for i == 0 [bucket_bounds[i-1], bucket_bounds[i]) for 0 < i < N-2 [bucket_bounds[i-1], +infinity) for i == N-1  i.e. an underflow bucket (number 0), zero or more finite buckets (1 through N - 2, and an overflow bucket (N - 1), with inclusive lower bounds and exclusive upper bounds.  If bucket_bounds has no elements (zero size), then there is no histogram associated with the Distribution. If bucket_bounds has only one element, there are no finite buckets, and that single element is the common boundary of the overflow and underflow buckets. The values must be monotonically increasing.  Don't change bucket boundaries within a timeseries if your backend doesn't support this.
     * @type {Array&lt;number&gt;}
     * @memberof MetricsDistributionValue
     */
...
export interface MetricsMetric {
    /**
     * The definition of the Metric. For now, we send the full MetricDescriptor every time in order to keep the protocol stateless, but this is one of the places where we can make future changes to make the protocol more efficient.
...
export interface MetricsPoint {
    /**
     * The moment when this point was recorded. If not specified, the timestamp will be decided by the backend.
     * @type {Date}
...
export interface MetricsTimeSeries {
    /**
     * Must be present for cumulative metrics. The time when the cumulative value was reset to zero. The cumulative value is over the time interval [start_timestamp, timestamp]. If not specified, the backend can use the previous recorded value.
     * @type {Date}
...
export interface SpanAttributes {
    /**
     * \"/instance_id\": \"my-instance\"     \"/http/user_agent\": \"\"     \"/http/server_latency\": 300     \"abc.com/myattribute\": true
...
/**
 * The relationship of the current span relative to the linked span: child, parent, or unspecified.   - TYPE_UNSPECIFIED: The relationship of the two spans is unknown, or known but other than parent-child.  - CHILD_LINKED_SPAN: The linked span is a child of the current span.  - PARENT_LINKED_SPAN: The linked span is a parent of the current span.
 * @export
 * @enum {string}
 */
...
/**
 * Type of span. Can be used to specify additional relationships between spans in addition to a parent/child relationship.   - SPAN_KIND_UNSPECIFIED: Unspecified.  - SERVER: Indicates that the span covers server-side handling of an RPC or other remote network request.  - CLIENT: Indicates that the span covers the client-side wrapper around an RPC or other remote request.
 * @export
 * @enum {string}
 */
...
/**
 * Indicates whether the message was sent or received.   - TYPE_UNSPECIFIED: Unknown event type.  - SENT: Indicates a sent message.  - RECEIVED: Indicates a received message.
 * @export
 * @enum {string}
 */
...
export const ExportApiFetchParamCreator = function (configuration?: Configuration) {
    return {
         ...
         */
        exportMetrics(body: ExporterExportMetricsRequest, options: any = {}): FetchArgs {
            // verify required parameter 'body' is not null or undefined
            if (body === null || body === undefined) {
                throw new RequiredError('body','Required parameter body was null or undefined when calling exportMetrics.');
            }
            ...
            const needsSerialization = (<any>"ExporterExportMetricsRequest" !== "string") || localVarRequestOptions.headers['Content-Type'] === 'application/json';
         ...
         * @throws {RequiredError}
         */
        exportSpan(body: ExporterExportSpanRequest, options: any = {}): FetchArgs {
            // verify required parameter 'body' is not null or undefined
            if (body === null || body === undefined) {
                throw new RequiredError('body','Required parameter body was null or undefined when calling exportSpan.');
            }
            ...
            delete localVarUrlObj.search;
            localVarRequestOptions.headers = Object.assign({}, localVarHeaderParameter, options.headers);
            const needsSerialization = (<any>"ExporterExportSpanRequest" !== "string") || localVarRequestOptions.headers['Content-Type'] === 'application/json';
            localVarRequestOptions.body =  needsSerialization ? JSON.stringify(body || {}) : (body || "");
...
export const ExportApiFp = function(configuration?: Configuration) {
    return {
         ...
         * @throws {RequiredError}
         */
        exportMetrics(body: ExporterExportMetricsRequest, options?: any): (fetch?: FetchAPI, basePath?: string) => Promise<ExporterExportMetricsResponse> {
            const localVarFetchArgs = ExportApiFetchParamCreator(configuration).exportMetrics(body, options);
         ...
         * @throws {RequiredError}
         */
        exportSpan(body: ExporterExportSpanRequest, options?: any): (fetch?: FetchAPI, basePath?: string) => Promise<ExporterExportSpanResponse> {
            const localVarFetchArgs = ExportApiFetchParamCreator(configuration).exportSpan(body, options);
            return (fetch: FetchAPI = portableFetch, basePath: string = BASE_PATH) => {
                return fetch(basePath + localVarFetchArgs.url, localVarFetchArgs.options).then((response) => {
                    if (response.status >= 200 && response.status < 300) {
                        return response.json();
...
export const ExportApiFactory = function (configuration?: Configuration, fetch?: FetchAPI, basePath?: string) {
    return {
        ...
        exportMetrics(body: ExporterExportMetricsRequest, options?: any) {
            return ExportApiFp(configuration).exportMetrics(body, options)(fetch, basePath);
        },
        /**
         * 
         * @param {ExporterExportSpanRequest} body  (streaming inputs)
        ...
        exportSpan(body: ExporterExportSpanRequest, options?: any) {
            return ExportApiFp(configuration).exportSpan(body, options)(fetch, basePath);
        },
    };
};
...
export class ExportApi extends BaseAPI {
    ...
    public exportMetrics(body: ExporterExportMetricsRequest, options?: any) {
        return ExportApiFp(this.configuration).exportMetrics(body, options)(this.fetch, this.basePath);
    }
    ...
    public exportSpan(body: ExporterExportSpanRequest, options?: any) {
        return ExportApiFp(this.configuration).exportSpan(body, options)(this.fetch, this.basePath);
    }
